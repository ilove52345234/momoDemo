...
public interface MemberRepository extends JpaRepository<MemberEntity, Long>, JpaSpecificationExecutor<MemberEntity> {
    ...
    default List<MemberEntity> findMembers(MemberCondition condition) {Specification<MemberEntity> spec = (root, query, builder) -> {
        ...
        if (condition.getStartDate() != null && condition.getEndDate() != null) {
            ...
            LocalDateTime startDate = condition.getStartDate().atStartOfDay();
//            predicates.add(builder.lessThanOrEqualTo(root.get("createdDate"), startDate));
            ...
            LocalDateTime endDate = condition.getEndDate().atTime(LocalTime.MAX);
//            predicates.add(builder.greaterThanOrEqualTo(root.get("createdDate"), endDate));
